...
def write_fortran_dope_type(file, rank):
    if rank > 0:
        file.write(
            'type, bind(c) :: dope{RANK:d}\n'
            '  type(c_ptr) :: base_addr\n'
            '  integer(c_size_t) :: elem_len\n'
            '  integer(c_int64_t) :: rank\n'
            '  integer(c_int64_t) :: type\n'
            '  type(dim_t), dimension({RANK:d}) :: dim\n'
            'end type\n'
            'public :: dope{RANK:d}\n\n'.format(RANK=rank)
        )
    else:
...
def write_c_definition(file, rank, ctype):
    if rank > 0:
        ...
        file.write((
            'void make_dope_{RANK:d}d_{TYPE:s}(dope<{TYPE:s},{RANK:d}>& d, {TYPE:s}& a) {{\n'
...
def main():
    ...
    with open(os.path.join(args.outdir, 'dope_generated_fortran_types.h'), 'w') as fortran_types:
        """
        Autogenerates the Fortran dope types from rank 0 to N, e.g., `type, bind(c) :: dope1`
        """
        write_generated_warning(fortran_types, fortran_comment)
        for rank in range(0, args.RANK+1):
            write_fortran_dope_type(fortran_types, rank)

    with open(os.path.join(args.outdir, 'dope_generated_fortran_interfaces.h'), 'w') as fortran_interfaces:
        """
        Autogenerates the Fortran `make_dope` inderface by writing, e.g., `make_dope_1d_c_int`, etc.
        """
    ...
    with open(os.path.join(args.outdir, 'dope_generated_fortran_definitions.h'), 'w') as fortran_definitions:
        """
        Autogenerates the Fortran function definitions for all the make_dope functions, e.g., `make_dope_1d_c_int`, etc.
        """
    ...
    with open(os.path.join(args.outdir, 'dope_generated_c_definitions.hpp'), 'w') as c_definitions:
        """
        Autogenerates the C definitions for all the make_dope functions, e.g., `make_dope_1d_int`, etc.
        These get implicitly called by the Fortran versions.
        """
    ...
    with open(os.path.join(args.outdir, 'dope_generated_type_identifier_traits.hpp'), 'w') as cpp_type_identifiers:
        """
        Autogenerates C++ type traits for verifyting the types of data pointed at by dope vectors.
        The values to which the type traits are assigned are compile definitions from the `ISO_Fortran_binding.h` header.
        """
